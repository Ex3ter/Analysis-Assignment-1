...
I chose this data set because I am just starting the game, and decided it would be a good source of relatively unbiased data as I have no experience with the game. I can also see how my choices affect my gameplay and if my win rate changes over time.  Additionally, it allowed me to use this assignment as a source of motivation to play the game, as it was now part of my grade.

These types of games are known for their complexity, so I will omit many of the details of how this game functions.  However, I will quickly cover the elements that appear in my data.  I have also included several screenshots of the game to act as visual aides.
...
Moving into the actual analysis, I need to start by cleaning up the data set.  There are several data points that are superfluous given the analysis methods being used, and should therefore be removed.
...
Make the data easier to reference by changing it's name to something easier to type.
```{r name fix}
data<-`2XKO.Data`
```

~~Steal some code from Stackoverflow to make the first row the header.~~  This used to be required in order to assign the first row as the header, but now it appears to set the first row to the header automatically.  I do not know why this has changed, but I will leave this old code here just in case it decides to revert itself.
```{r clean header}
...
```{r remove columns}
data<-subset(data,select = -c(4,5,6))
```
...
So, let's start with a simple t-test.  I'm curious as to whether my win rate is 50% (average in a 1v1 competitive sport) or not.  So H0:mu=0.5, H1:mu=/0.5.

I was going to try and be fancy and convert column 8 to a clean win/loss record, but it's a lot less work to just go through the original file and add the column manually.
```{r t}
print(data$Win.)
print(mean(data$Win.))
#60%.
t.test(data$Win., mu=0.5)
```
A p-value of 0.33 is pretty conclusive.  I fail to reject the null.  There is no evidence to suggest that my win rate is not 50%.
...
Then run the test.
```{r fisher}
#A quite test function to make sure the Fisher test isn't wildly off base
dhyper(x=3, m=15, n=10, k=6)
#0.31.  Expect a larger p-value than this
fisher.test(fuse)
```
...
I'm not entirely sure if this is a viable usage of Linear Regression, but I'm going to try and see if game number or match duration correlate with winning.
...
#Make the list
list<-c(26, 52.3, 56.7, 53.7, 54, 76, 77.3, 65.5, 76, 78, 79, 64, 65, 74.5, 60.5, 61.3, 69.5, 64, 65, 64.3, 60.5, 54, 61, 62.5, 52)
#Merge it with the original dataset
data$Average_match_duration <- list
#Check it worked
head(data)
```
...
```{r linplot}
plot(data$Game_number,data$Average_match_duration)
```
I could argue a vague downward trend, but it's pretty random.

Now to analyse.
```{r linear_regression}
#Run the linear regression
summary(lm(Win.~Game_number*Average_match_duration,data=data))
```

Not unexpected. I wasn't seeing much of a relation in the plot either, so I didn't expect one to materialize here.

I guess I could try logging it to see if that changes anything, but I doubt it.

```{r loglinplot}
summary(lm(Win.~log(Game_number)*log(Average_match_duration),data=data))
```

As expected.  If anything it got a touch worse.  I didn't log my wins because that creates errors.
